 ...
 * @brief debugger Live Expression용 선택형 current/speed command source를 정의한다.
 *
...
/**
 * @brief debugger command source가 시작할 drive mode.
 */
typedef enum {
    DRIVE_DEBUG_COMMAND_MODE_CURRENT = 0, /**< Current control을 시작한다. */
    DRIVE_DEBUG_COMMAND_MODE_SPEED /**< Speed control을 시작한다. */
} drive_debug_command_mode_t;

/**
 * @brief debugger가 읽고 쓰는 drive command source 상태.
 *
 * requested_mode, requested_i_d_a, requested_i_q_a, requested_speed_rpm, start_requested,
 * stop_requested만 debugger에서 쓴다. 나머지는 관찰용 결과이며 App 또는 motor control의 canonical
 * state를 복제하지 않는다.
 */
typedef struct {
    volatile drive_debug_command_mode_t requested_mode; /**< 다음 start에 적용할 drive mode. */
    volatile float requested_i_d_a; /**< current mode의 d축 current 지령 [A]. */
    volatile float requested_i_q_a; /**< current mode의 q축 current 지령 [A]. */
    volatile float requested_speed_rpm; /**< 다음 start 또는 실행 중 SET_SPEED의 요청 속도 [rpm]. */
    volatile bool start_requested; /**< true이면 READY에서 선택한 mode의 시작을 한 번 요청한다. */
    volatile bool stop_requested; /**< true이면 실행 중인 mode의 정상 정지를 한 번 요청한다. */
    volatile dq_t applied_i_dq_a; /**< 마지막으로 수락된 d/q current target [A]. */
    volatile float applied_speed_rpm; /**< 마지막으로 수락된 speed target [rpm]. */
    volatile bool is_running; /**< App이 current/speed drive 상태이면 true. */
 ...
 * @note 이 함수는 speed command를 ±3000 rpm으로 제한한다. current command의 최종 제한과 rate
 *       limit은 motor_control이 적용한다. ISR에서는 호출하지 않는다.
 */
